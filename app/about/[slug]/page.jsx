...
import { teamMembers, getTeamMember } from "@/lib/teamConfig";
import { advisors, getAdvisor } from "@/lib/advisorConfig";
...
export default async function PersonPage({ params }) {
  const { slug } = await params;
  const person = getTeamMember(slug) || getAdvisor(slug);

  if (!person) notFound();

  const isTeamMember = !!getTeamMember(slug);
  ...
  return (
    <main id="main-content" className="flex-1">
              ...
              <div className="text-primary/80 leading-relaxed font-light max-w-none space-y-6">
                {person.bio.map((paragraph, i) => (
                  <p
                    key={i}
                    className={
                      person.lastParagraphBold && i === person.bio.length - 1
                        ? "font-medium"
                        : undefined
                    }
                  >
      ...
      {/* Rest of the Team (only for team members) */}
      {isTeamMember && <TeamMembersGrid currentSlug={person.slug} />}
